function W = stiffnessMatrix(vertex,face,type,options)
    ...
    case 'conformal'
        ...
        for i = 1:n
            ...
            for b = ring{i}
                ...
                %add weight--modifica con -0.5
                W(i,j) = W(i,j) + cot( alpha )*-0.5;
                W(i,k) = W(i,k) + cot( beta )*-0.5;
            end
        end
        %modifica: quando i=j
        for i=1:n
            W(i,i) = -(sum(W(i,1:i-1)) + sum(W(i,i+1:end)));
        end
